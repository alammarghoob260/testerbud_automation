...
class FormPage {
  constructor(page) {
    ...
    this.successMessage = page.locator(
      '.card-body > div, .alert, [data-testid="success"]'
    ).first();
  }
}
```
...
const matches = await page.locator('#email').evaluate(el => 
  document.querySelectorAll('input[type="email"]').includes(el)
);
```
